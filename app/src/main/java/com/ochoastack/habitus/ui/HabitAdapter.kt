    ...
    override fun onBindViewHolder(holder: HabitVH, position: Int) {
        ...
        with(holder.binding) {
            ...
            if (habit.estaCompletadoHoy) {
                ivHabitIcon.background = root.context.getDrawable(com.ochoastack.habitus.R.drawable.bg_circle_completed)
                ivHabitIcon.imageTintList = android.content.res.ColorStateList.valueOf(android.graphics.Color.WHITE)
            } else {
                ivHabitIcon.background = root.context.getDrawable(com.ochoastack.habitus.R.drawable.bg_circle_empty)
                ivHabitIcon.imageTintList = android.content.res.ColorStateList.valueOf(root.context.getColor(com.ochoastack.habitus.R.color.accent))
            }
