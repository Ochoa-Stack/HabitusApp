...
class HomeFragment : Fragment() {
    ...
    private fun mostrarBalanceCognitivo(balance: Map<String, Int>) {
        ...
        com.ochoastack.habitus.data.TipoCognitivo.todos.forEach { tipo ->
            val cantidad = balance[tipo] ?: 0
            if (cantidad == 0) return@forEach

            val porcentaje = (cantidad * 100f / total).toInt()
            val colorRes   = com.ochoastack.habitus.data.TipoCognitivo.colorRes(tipo)
            val emoji      = com.ochoastack.habitus.data.TipoCognitivo.emoji(tipo)
